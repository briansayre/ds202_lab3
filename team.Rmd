```{r}
library(dplyr)
library(tidyverse)
library(ggplot2)
data <- readxl::read_xls('./GSS.xls')
str(data)
df <- data.frame(
  year=data$`Gss year for this respondent`, 
  happy=data$`General happiness`, 
  party=data$`Political party affiliation`, 
  income=data$`Total family income`, 
  race=data$`Race of respondent`, 
  sex=data$`Respondents sex`, 
  school=data$`Highest year of school completed`, 
  mstatus=data$`Marital status`, 
  id=data$`Respondent id number`, 
  ballot=data$`Ballot used for interview`) 
str(df)
```


### Question 1. 
<p>I chose to display the data as a boxplot for each marriage status. I chose this to show as mush information about the education for each status as possible. We can see that the ones with no answer seem to have the highest mean education which is something I did not expect. We also can see the rest have around the same mean, but a difference in most of their interquartile ranges. Divorced, married and never married seem to have a higher IQR than separated and widowed.</p>

```{r}
q1df <- as.data.frame(df)
q1df <- filter(q1df, school != 'No answer' & school != "Don't know" & !is.na(school) & school != "NA" )
unique(q1df$school)
q1df$school <- as.numeric(q1df$school)
ggplot(q1df, aes(y=school, x=mstatus), fill=school) + geom_boxplot()
```

```{r}
q1df %>% 
  group_by(mstatus) %>% 
  summarize(
    mean_rate = mean(school, na.rm=TRUE),
    sd_rate = sd(school, na.rm=TRUE),
    nrows = n()
  )
```

### Question 2. 
```{r}
q2df <- q1df %>% 
  group_by(mstatus) %>% 
  filter(school > mean(school))
str(q2df)
```


### Question 3
```{r}
df2 <- as.data.frame(df)
str(df2)
happy_df <- df2 %>% 
  group_by(mstatus) %>%
  filter(happy=="Pretty happy" | happy=="Very happy") %>%
  filter(mstatus!="No answer")

unhappy_df <- df2 %>%
  group_by(mstatus) %>%
  filter(happy=="Not too happy") %>%
  filter(mstatus!="No answer")

unhappy_table <- unhappy_df %>%
  group_by(mstatus) %>%
  summarize(
    Total_Unhappy=n()
  )
unhappy_table

happy_table <- happy_df %>%
  group_by(mstatus) %>%
  summarize(
    Total_Happy=n()
  )
happy_table

ggplot(happy_df, aes(x=mstatus)) + geom_bar(color="blue", fill="red")
ggplot(unhappy_df, aes(x=mstatus)) + geom_bar(color="blue", fill="red")

mosaicplot(happy_df$happy~happy_df$mstatus, shade=TRUE, las=1)
mosaicplot(unhappy_df$happy~unhappy_df$mstatus, shade=TRUE, las=1)
```
<p>
It would seem that there are more people married and happy than there are of any other group. Never married people also seem much happier than divorced, separated, or divorced people. </p>

### Question 4
```{r}
unique(happy_df$party)

happy_indep_df <- happy_df %>%
  group_by(mstatus) %>%
  filter(party=="Independent" | party=="Ind,near rep" | party=="Ind,near dem")
happy_indep_df
ti1 <- happy_indep_df %>%
  summarize(
    Total_Happy_Indep=n()
  )
ti1

unhappy_indep_df <- unhappy_df %>%
  group_by(mstatus) %>%
  filter(party=="Independent" | party=="Ind,near rep" | party=="Ind,near dem")
unhappy_indep_df
ti2 <- unhappy_indep_df %>%
  summarize(
    Total_Unhappy_Indep=n()
  )
ti2

happy_rep_df <- happy_df %>%
  group_by(mstatus) %>%
  filter(party=="Strong republican" | party=="Not str republican")
t1 <- happy_rep_df %>%
  summarize(
    Total_Happy_Rep=n()
  )
t1

unhappy_rep_df <- unhappy_df %>%
  group_by(mstatus) %>%
  filter(party=="Strong republican" | party=="Not str republican")
t2 <- unhappy_rep_df %>%
  summarize(
    Total_Unhappy_Rep=n()
  )
t2

happy_dem_df <- happy_df %>%
  group_by(mstatus) %>%
  filter(party=="Strong democrat" | party=="Not str democrat")
t3 <- happy_dem_df %>%
  summarize(
    Total_Happy_Rep=n()
  )
t3

unhappy_dem_df <- unhappy_df %>%
  group_by(mstatus) %>%
  filter(party=="Strong democrat" | party=="Not str democrat")
t4 <- unhappy_dem_df%>%
  summarize(
    Total_Unhappy_Rep=n()
  )
t4

ggplot(happy_rep_df, aes(x=party)) + geom_bar(color="blue", fill="red")
ggplot(unhappy_rep_df, aes(x=party)) + geom_bar(color="blue", fill="red")
ggplot(happy_dem_df, aes(x=party)) + geom_bar(color="red", fill="blue")
ggplot(unhappy_dem_df, aes(x=party)) + geom_bar(color="red", fill="blue")
ggplot(happy_indep_df, aes(x=party)) + geom_bar(color="black", fill="purple")
ggplot(unhappy_indep_df, aes(x=party)) + geom_bar(color="black", fill="purple")
```
<h3>Conclusion</h3>
<p>
It would seem that there are similar amounts of happy and unhappy democrats as well as happy and unhappy republicans as well as happy and unhappy independents. This would seem to suggest party affiliation is not a factor in happy.</p>

### Question 6b. 
<p>Does income affect your overall happy?
The graph and data show that as a persons income increases the average happy rating increases. For the data, i used "Not too happy" = 1, "Pretty happy" = 2, "Very happy" = 3. This allowed me to run an average of each person for each income range. Although the happy slightly increase as income increases, we can see that they are all still right around a 2 average, which is pretty happy.</p>

```{r}
q6df <- select(df,happy, income)
q6df <- filter(q6df, happy != "No answer" & happy != "Don't know" & happy != "Not applicable")
q6df <- filter(q6df, income != "No answer" & income != "Don't know" & income != "Not applicable" & income != "Refused")
q6df$happyNum <- recode(q6df$happy, "Not too happy" = 1, "Pretty happy" = 2, "Very happy" = 3)
q6df <- aggregate(q6df$happyNum, by=list(q6df$income), FUN=mean)
q6qf <- rename(q6df, Income = Group.1, happy = x)
ordered <- c("Lt $1000", "$1000 to 2999", "$3000 to 3999", "$4000 to 4999", "$5000 to 5999", "$6000 to 6999", "$7000 to 7999", "$8000 to 9999", "$10000 - 14999", "$15000 - 19999", "$20000 - 24999", "$25000 or more")
q6qf$Income <- factor(q6qf$Income, levels=ordered)
```
```{r}
ggplot(q6qf, aes(y=happy, x=Income)) + geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = -90))+ ylim(0, 3)
```
```{r}
q6qf
```


